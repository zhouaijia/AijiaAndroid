...
public final class EncryptUtils {
    ...
    public static String xor(String content) {
        String key = EncryptUtils.encryptMD5ToString("~1sw3@1!3bmvd8!@9d~*%");

        char[] bytes = content.toCharArray();
        char[] keyBytes = key.toCharArray();

        for (int i = 0; i < keyBytes.length; i++)
            keyBytes[i] = (char) (keyBytes[i] ^ (keyBytes.length - i));

        int j = 0;
        for (int i = 0; i < bytes.length; i++) {
            if (j > keyBytes.length - 1)
                j = 0;
            bytes[i] = (char) (bytes[i] ^ keyBytes[j]);
        }
    ...
    /**
     * MD5加密字符串
     * @param bit 需要取多少位，如16、32位
     * @param str 需加密的字符串
     * @return 加密为md5后的字符串
     */
    public static String string2Md5(int bit, String str){
        try{
            MessageDigest md = MessageDigest.getInstance("MD5");
