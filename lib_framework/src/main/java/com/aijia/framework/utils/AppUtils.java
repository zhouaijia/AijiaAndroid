...
public final class AppUtils {
     ...
     * 安装APK程序
     */
    public static void installApk(final Context context, String apkPath, String apkName) {
        ...
        try {
            ...
            new Thread(() -> {
                Looper.prepare();
                ToastUtils.longToast(context, "文件安装失败,请到文件管理根目录找到RekTecXmobile.apk文件，手动点击安装");
                Looper.loop();
            }).start();
    ...
    public static String getMetadataValue(String key) {
        try {
            String packageName = getPackageName(Utils.getApp());
            ApplicationInfo ai = Utils.getApp().getPackageManager().getApplicationInfo(packageName, PackageManager.GET_META_DATA);
